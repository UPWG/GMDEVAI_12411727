...
public class AIControl : MonoBehaviour
{
    ...
    void Wander()
    {
        float wanderRadius = 12;
        float wanderDistance = 6;
    ...
    void Hide()
    {
        ...
        int hidingSpotsCount = World.Instance.GetHidngSpots().Length;
        for (int i = 0; i < hidingSpotsCount; i++)
        {
            Vector3 hideDirection = World.Instance.GetHidngSpots()[i].transform.position - target.transform.position;
            Vector3 hidePosition = World.Instance.GetHidngSpots()[i].transform.position + hideDirection.normalized * 5;
    ...
    void CleverHide()
    {
        ...
        GameObject chosenGameObject = World.Instance.GetHidngSpots()[0];
        ...
        for (int i = 0; i < hidingSpotsCount; i++)
        {
            ...
            if (spotDistance < distance)
            {
                ...
                chosenGameObject = World.Instance.GetHidngSpots()[i];
                distance = spotDistance;
            }
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            playerMovement = other.GetComponent<WASDMovement>();
        }
    }
